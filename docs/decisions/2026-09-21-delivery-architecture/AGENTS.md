...
## This is a DECISION PROJECT, and the project root is this directory

`docs/decisions/2026-09-21-delivery-architecture/` is a self-contained research project
nested inside a larger repository. Two rules follow, and both have been broken already:

1. **Every kit command runs with THIS directory as the current working directory.** The kit
   takes no project argument. Run from the repository root, `preflight` judges the
   repository's corpus - a different contract, a different set of unknowns - and reports a
   verdict that says nothing about this project. Quoting the parent's PASS beside this
   project's claims is the error that produced
   `../../preliminary-2026-09-21-delivery-architecture.md`.
2. **This project's corpus is committed, ledger included.** Research supporting a
   repository-level architectural decision lives at `docs/decisions/<date>-<name>/`.
   ...
   preserved. The first attempt at this research was collected into a temporary directory
   and deleted as cleanup, taking nine captures, the hash chain and the provenance for
   every claim with it.

Do not collect this project's topic into the repository's own `research/`, and do not
collect the repository's topic into this one. One project, one contract, one corpus.
...
0. **Decompose** (collector machine): `node ../../../research-kit/bin/decompose.mjs --topic "<topic>"`
...
2. **Collect**: `node ../../../research-kit/bin/research.mjs --plan research/plan.json`. Prefer the page
...
4. **Gate**: `node ../../../research-kit/bin/preflight.mjs`. **Do not start building until it prints
...
cache (`--refresh-days`), and check the budget with `node ../../../research-kit/bin/research.mjs --status`.
