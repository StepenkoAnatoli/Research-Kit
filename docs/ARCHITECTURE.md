...
1. **Transport seam** — `runScrape`/`search`/`scrape`/`command`/`map` are injected
   ...
   the usage log, the failure log, and the `rankedBy` of any URL it promoted — because a
   ranking is a different provenance claim from a fetch, and it never enters the hash
   chain. A `cmd` string is an annotation and proves nothing.
