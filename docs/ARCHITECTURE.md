...
> **Two rows above describe modules that do not exist here.**
>
> `release-validator.mjs` and `path-authority-validator.mjs` are part of the
> release-evidence validator layer that ADR-0022 deliberately deferred. This table
> described them as ordinary modules, with no marker, from the day it was written —
> so a reader scanning it had no way to tell them from the 27 rows that are real.
> `research-kit/README.md` said the layer was not built; this file did not, and this
> file is the one that claims to be the map of the code.
>
> ADR-0022 had a reason for leaving it unmarked: the file was a byte-preserved bundle
> source, so the delta was recorded in the build report instead. **ADR-0028 superseded
> exactly that clause on 2026-09-20**, which removed the reason without anybody
> noticing the consequence. Marked now.
>
> An implementation of this layer exists elsewhere on the author's machine
> (1,700 lines, 12 schemas, 12 test files, 61 passing) but is **not in this
> repository**, has no git history, and has not been reviewed against these specs.
> Until it is ported and verified, these two rows are a specification, not a map.
