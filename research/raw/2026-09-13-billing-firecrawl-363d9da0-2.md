---
url: https://docs.firecrawl.dev/billing
retrieved: 2026-09-13T17:38:25.146Z
date: 2026-09-13
type: P
host: docs.firecrawl.dev
title: "Billing | Firecrawl"
...
command: firecrawl scrape https://docs.firecrawl.dev/billing --only-main-content --json # transport: agent page fetch (no Firecrawl egress in this sandbox; no credits spent)
statusCode: 200
completeness: full
---
> ## Documentation Index
...
[Skip to main content](https://docs.firecrawl.dev/billing#content-area)

## [​](https://docs.firecrawl.dev/billing\#overview)  Overview

Firecrawl billing is **credit-based**. Every API call that you make consumes credits. The number of credits depends on the endpoint and the options that you use. Your plan gives you a monthly credit allotment. Pay-as-you-go adds more credits when the allotment runs out.For current plan pricing, visit the [Firecrawl pricing page](https://www.firecrawl.dev/pricing).

All Firecrawl invoices are billed in **US Dollars (USD)**, regardless of your billing address or payment method.

## [​](https://docs.firecrawl.dev/billing\#credits)  Credits
...
### [​](https://docs.firecrawl.dev/billing\#credit-costs-per-endpoint)  Credit costs per endpoint
...
### [​](https://docs.firecrawl.dev/billing\#additional-credit-costs-for-scrape-options)  Additional credit costs for scrape options
...
| Prompt injection check | +4 credits / page | Opt-in `checkPromptInjection` guard for JSON format (see [Prompt injection detection](https://docs.firecrawl.dev/features/llm-extract#prompt-injection-detection)). If the scrape fails after the check has run, 5 credits are billed. See [When credits are charged](https://docs.firecrawl.dev/billing#when-credits-are-charged). |
| Zero Data Retention (ZDR) | +1 credit / page | Ensures no data is persisted beyond the request (see [Scrape ZDR](https://docs.firecrawl.dev/features/scrape#zero-data-retention-zdr)) |
| `question` or `query` format | +4 credits / page | LLM-generated answer to a question about the page (see [Scrape](https://docs.firecrawl.dev/features/scrape)) |
| `highlights` format | +4 credits / page | LLM-selected relevant passages from the page (see [Scrape](https://docs.firecrawl.dev/features/scrape)) |
| `audio` format | +4 credits / page | Transcribe audio found on the page (see [Scrape](https://docs.firecrawl.dev/features/scrape)) |
| `video` format | +4 credits / page | Transcribe video found on the page (see [Scrape](https://docs.firecrawl.dev/features/scrape)) |
| PII redaction (`redactPII`) | +4 credits / page | Redact personal data from the returned markdown. Each additional PDF page adds another +4 on top of its +1 parsing cost (see [PII redaction](https://docs.firecrawl.dev/features/pii-redaction)) |
| `lockdown` (cache hit) | +4 credits / page | Serve from cache only, never fetching the target. A cache miss returns no document and bills 1 credit (see [Lockdown](https://docs.firecrawl.dev/features/lockdown) and the table below) |
| Enhanced proxy (`proxy: "enhanced"` or `auto` escalation) | +0 | Billed at the same 1 credit as a basic request. An escalated retry is not charged separately (see [Enhanced Mode](https://docs.firecrawl.dev/features/enhanced-mode)) |

These modifiers stack. For example, scraping a page with both JSON format and Zero Data Retention costs **1 + 4 + 1 = 6 credits** per page, and JSON format with PII redaction costs **1 + 4 + 4 = 9 credits**. These same modifiers apply to the Crawl and Search endpoints since they use scrape internally for each page.Requests to `x.com` and other X/Twitter URLs use the Grok API and have separate pricing. See [X (x.com) billing](https://docs.firecrawl.dev/billing#x-xcom-billing) at the bottom of this page.

### [​](https://docs.firecrawl.dev/billing\#when-credits-are-charged)  When credits are charged
...
- **API**: Use the [Credit Usage](https://docs.firecrawl.dev/api-reference/endpoint/credit-usage) and [Credit Usage Historical](https://docs.firecrawl.dev/api-reference/endpoint/credit-usage-historical) endpoints to programmatically check your usage

We are actively working on improvements to make credit usage easier to understand. Stay tuned for updates.

## [​](https://docs.firecrawl.dev/billing\#plans)  Plans

Subscription plans bill monthly or yearly. Paid self-serve plans can also use pay-as-you-go. Pay-as-you-go adds credits when your plan allotment runs out. See [Pay-as-you-go](https://docs.firecrawl.dev/billing#pay-as-you-go).

### [​](https://docs.firecrawl.dev/billing\#paid-plans)  Paid plans
...
### [​](https://docs.firecrawl.dev/billing\#free-plan)  Free plan
...
- **Rate limits of 10 requests per minute** on `/scrape`, `/map` and `/search`, and **2 requests per minute** on `/crawl`, `/agent` and `/interact`. Batch scrape shares the scrape limit and extract shares the agent limit. See [Rate Limits](https://docs.firecrawl.dev/rate-limits) for the full table.
- **Every core product endpoint.** Scrape, crawl, map, search, extract, batch scrape, interact and monitor all work on the free plan. Some team administration and enterprise features are enabled per team rather than by plan, including threat protection, SIEM logging and zero data retention search. Credit costs are the same on every plan, so the [credit costs per endpoint](https://docs.firecrawl.dev/billing#credit-costs-per-endpoint) table above applies unchanged.

The free plan does not include pay-as-you-go, so requests return an HTTP 402 once the monthly allotment runs out. See [Running Out of Credits](https://docs.firecrawl.dev/billing#running-out-of-credits). It also does not include credit rollover, a Data Processing Agreement, or Slack support. Support is community support.Scrape, Search and Interact also work with no API key at all, through the keyless free tier, which is capped per IP address per day rather than by a credit allotment. See [Keyless (no API key)](https://docs.firecrawl.dev/rate-limits#keyless-no-api-key).

### [​](https://docs.firecrawl.dev/billing\#billing-cycle)  Billing cycle
...
### [​](https://docs.firecrawl.dev/billing\#concurrent-browsers)  Concurrent browsers

Concurrent browsers represent how many web pages Firecrawl can process for you simultaneously. Your plan determines this limit. If you exceed it, additional jobs wait in a queue until a slot opens. See [Rate Limits](https://docs.firecrawl.dev/rate-limits) for full details on concurrency and API rate limits.

## [​](https://docs.firecrawl.dev/billing\#pay-as-you-go)  Pay-as-you-go

Pay-as-you-go keeps your requests running when your plan credits run out. It adds credits to your account automatically.You buy credits in increments of 5 USD. When your credit balance reaches zero, we add one increment to your account and charge your card on file.Pay-as-you-go needs a paid self-serve plan. You cannot use pay-as-you-go on the free plan.You can also buy credits yourself at any time. Use **Load more credits** in your [billing settings](https://www.firecrawl.dev/app/settings?tab=billing). Enter a multiple of 5 USD, and pay with your card on file. This works whether pay-as-you-go is on or off.

### [​](https://docs.firecrawl.dev/billing\#credits-in-a-batch)  Credits in an increment
...
### [​](https://docs.firecrawl.dev/billing\#set-the-monthly-auto-reload-limit)  Set the monthly pay-as-you-go limit
...
### [​](https://docs.firecrawl.dev/billing\#how-the-limit-caps-your-monthly-spend)  How the limit caps your monthly spend

Your limit is the most that pay-as-you-go can spend in one month. It rounds down to whole 5 USD increments.For example, a limit of 25 USD allows five increments each month. A limit of 22 USD allows four increments, because 22 USD rounds down to 20 USD.Credits that you buy manually do not count toward this limit.

## [​](https://docs.firecrawl.dev/billing\#upgrading-and-downgrading)  Upgrading and Downgrading
...
## [​](https://docs.firecrawl.dev/billing\#running-out-of-credits)  Running Out of Credits
...
While pay-as-you-go is on, requests are not cut off at exactly zero. Your plan carries an overage allowance so that a burst of requests keeps running while a top-up settles: **1,500 credits on Hobby, 30,000 on Standard, 150,000 on Growth, and 600,000 on Scale**. Once the monthly limit is reached and the allowance is used, requests return **HTTP 402** until the next billing cycle. The allowance is headroom, not extra credits you keep.Manually purchased credits ( **Load more credits**) are spent before any of this applies and do not count toward the monthly pay-as-you-go limit.To resume usage after a hard stop, you can:

1. Set a **Monthly pay-as-you-go limit** to buy credits automatically. See [Pay-as-you-go](https://docs.firecrawl.dev/billing#pay-as-you-go).
2. Upgrade to a higher plan manually
3. Wait for your credits to reset at the next billing cycle

## [​](https://docs.firecrawl.dev/billing\#coupons)  Coupons
...
## [​](https://docs.firecrawl.dev/billing\#faqs)  FAQs
...
How does the monthly pay-as-you-go limit work?
...
Check the dashboard at [firecrawl.dev/app](https://www.firecrawl.dev/app), or call the [Credit Usage API endpoint](https://docs.firecrawl.dev/api-reference/endpoint/credit-usage) programmatically.
...
Go to your [billing settings](https://www.firecrawl.dev/app/settings?tab=billing). Team admins can manage everything there. Click **Manage Subscription** to open the billing portal and update your payment method, billing address, company name, or VAT number.To change plans, click **Change Plan** and pick a new tier. Upgrades take effect immediately. Downgrades take effect at the end of your current billing period, and you can undo one until then. See [Upgrading and Downgrading](https://docs.firecrawl.dev/billing#upgrading-and-downgrading).To cancel, click **Cancel Subscription**. Your plan stays active until the end of your current billing period, and you can resume it before then. When your plan ends, the credits you bought expire with it, so use them before you leave.

How do I add a VAT number, company name, or billing address to my invoices?

Go to your [billing settings](https://www.firecrawl.dev/app/settings?tab=billing), click **Manage Subscriptions**, and update your billing address, company name, and VAT number in the Stripe portal. Future invoices will automatically include the updated details.To regenerate a past paid invoice with the new information:

1. Update your billing details in the Stripe portal first (see above).
2. Open the **Invoice history** tab in the Stripe console and download the PDF you want; Stripe re-renders it against your current billing info.
3. Open an invoice that doesn’t pick up the updated details, email [help@firecrawl.dev](mailto:help@firecrawl.dev) with the invoice numbers and we’ll regenerate them for you.

## [​](https://docs.firecrawl.dev/billing\#x-x-com-billing)  X (x.com) billing
...
### [​](https://docs.firecrawl.dev/billing\#credit-costs)  Credit costs
...
For example, processing a typical post or thread request costs **30 credits** (`1` base + `29` Grok X Query) and returns Grok-generated structured data, thread context, and summaries. If JSON format (LLM extraction) is also enabled, the total is **34 credits** per request.This method complies with X’s published interfaces via xAI’s partnership and provides higher-quality, reasoned output instead of raw page scraping.
...
[Suggest edits](https://github.com/firecrawl/firecrawl-docs/edit/main/billing.mdx) [Raise issue](https://github.com/firecrawl/firecrawl-docs/issues/new?title=Issue%20on%20docs&body=Path:%20/billing)
